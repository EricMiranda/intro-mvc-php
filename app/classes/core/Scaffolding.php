...
#==================================================#
	...
	class Scaffolding
	{
		...
		public function metLeer()
		{
			...
			$cabecera="public function leer(\$id)";
			$contenido = "\n\t\t{\n\t\t\t\$id=(int)\$id;\n\t\t\t";
			$parametros ="\$r1 = \$this->db->select(\"$this->tabla\",\"id";
			$datos = "if(\$r1!=null)\n\t\t\t{\n\t\t\t\t\$r1=\$r1[0];\n\t\t\t\t\$this->id=\$id;\n\t\t\t\t";
			foreach($this->columnas as $field)
			{
				if($field != 'id')
		...
		public function metLeerTodos()
		{
			...
			$select = "\$r1 = \$db->select(\"{$this->tabla}\", \"id\", \"id>? order by id asc\", array(0));";
			$contenido = "\n\t\t\t\$datos = array();\n\t\t\t".
			"if (count(\$r1) > 0)\n\t\t\t".
			"{\n\t\t\t\t".
		...
		public function metUpdate()
		{
			...
			$pie="return \$db->update(\"{$this->tabla}\",\$dato);";
		...
		public function buildMetodo()
		{
			...
			$ruta_metodo = APP_BIN_PATH."/classes/".ucfirst($this->metodo).".php";
